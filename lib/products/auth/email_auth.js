...
import { ADMIN_DB_NAME, ADMIN_DB_URL, AUTH_PROVIDER_ID, EnginePath, REFRESH_TOKEN_EXPIRY, TOKEN_EXPIRY } from "../../helpers/values";
...
export const signupCustom = async (email = '', password = '', signupMethod = AUTH_PROVIDER_ID.PASSWORD, profile = {}, projectName, customExtras = {}) => {
    ...
    try {
        ...
        if (signupMethod === AUTH_PROVIDER_ID.PASSWORD) {
            if (!password) throw simplifyError('password_required', 'Password length must be greater than one');
...
export const signinCustom = async (email = '', password = '', signinMethod = AUTH_PROVIDER_ID.PASSWORD, projectName, defaultRecord) => {
    ...
    if (signinMethod === AUTH_PROVIDER_ID.PASSWORD) {
        if (!password) throw simplifyError('password_required', 'Password length must be greater than one');
        if (!Validator.EMAIL(email)) throw simplifyError('invalid_email_format', 'Please provide a valid email format');

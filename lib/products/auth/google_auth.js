import { decodeBinary, timeoutFetch } from "../../helpers/utils";
...
export const doGoogleSignin = async ({ googleAuthConfig, token, projectName, mergeAuthAccount }, req, interceptNewAuth, metax) => {
    ...
    if (isIdToken(token)) {
        userInfo = await tokenVerifier(token, clientID.split(',').map(v => v.trim()));
    } else {
        ...
        userInfo = JSON.parse(decodeBinary(token.split('.')[1]));
    }

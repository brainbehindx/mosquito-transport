...
import { ADMIN_DB_NAME, ADMIN_DB_URL, EnginePath, EngineRoutes, ERRORS, NO_CACHE_HEADER } from "../../helpers/values.js";
...
import { doAppleSignin } from "./apple_auth.js";
...
export const authRoutes = ({
    ...
    express.Router({ caseSensitive: true }).post(`/${mroute}`, async (req, res) => {
        ...
        try {
            ...
            switch (route) {
                case _customSignup:
                    const [email, password, name = ''] = data.split('.').map(v => decodeBinary(v));
                    const result = await signupCustom(email, password, undefined, { email, name: name.trim() || '' }, projectName, { req, metadata: metax, name });
                ...
                case _appleSignin:
                    ...
                    res.status(200).send(await makeResult({ status: 'success', result: r6 }));
                    break;
            }
        } catch (e) {
            if (

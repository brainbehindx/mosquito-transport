...
import { ADMIN_DB_NAME, ADMIN_DB_URL, AUTH_PROVIDER_ID, EnginePath, EngineRoutes, ERRORS } from "../../helpers/values.js";
...
export const authRoutes = ({
    ...
    ...enforceETE_Encryption ? [] : authRouteName.map(v => ({ mroute: v, route: v })),
    ...
    express.Router({ caseSensitive: true }).post(`/${mroute}`, async (req, res) => {
        ...
        try {
            if (internals?.auth === false) throw ERRORS.DISABLE_FEATURE;
            ...
            useDDOS(ddosMap, ddosRouting, req.ip, 'auth');
            ...
            switch (route) {
                case _customSignup:
                    ...
                    const { metadata = metax || {}, profile, uid: d_uid } = (await sneakSignupAuth?.(aBuild)) || {};
...
export const authLiveRoutes = ({ projectName, logger, enforceETE_Encryption, internals }) => [
                ...
                listener = listenUserVerificationState(async verified => {
                    try {
                        ...
                        socket.emit('onVerificationChanged', [
                            undefined,
                            await serializeE2E(!!verified, clientPublicKey, projectName)
                        ]);
                    } catch (e) {
...
export const listenUserVerificationState = (callback, onError, uid, projectName) => {
    ...
    readDocument({ path: EnginePath.userAcct, find: { _id: uid } }, projectName, ADMIN_DB_NAME, ADMIN_DB_URL).then(f => {
        if (!hasEmit && !hasCancelled) {
            if (!f) {
                onError?.(ERRORS.TOKEN_USER_NOT_FOUND);
                emittion();
                hasCancelled = true;
                return;
            }

                    ...
                    (scope === 'update' || scope === 'merge' || scope === 'updateOne' || scope === 'mergeOne')
                        ...
                        scope === 'setMany' ? k.insertMany(deserializedValue) :
                            (scope === 'updateMany' || scope === 'mergeMany') ? k.updateMany({ ...find }, { ...deserializedValue }, { upsert: scope === 'merge' })
                                :
                                scope === 'replaceOne' ?
                                    k.replaceOne({ ...find }, { ...value }) :
                                    scope === 'putOne' ?
                                        k.replaceOne({ ...find }, { ...value }, { upsert: true })
    ...
    tv.forEach((e, i) => {
        queryEntries(e).forEach(([key, value]) => {
            ...
            if (key.endsWith('$timestamp')) {
                const now = value === 'now' ? { $type: 'date' } : Timestamp.fromNumber(value)
                _.set(result[i], l.map((v, i, a) => i === a.length - 1 ? `$currentDate.${v}` : v).join('.'), now);
...
export const databaseRoutes = ({ projectName }) => [
    ...
    express.Router({ caseSensitive: true }).post(`/${route}`, async (req, res) => {
        try {
            ...
            console.error('databaseRoutes err:', e);
            res.status(403).send({ status: 'error', ...(e.simpleError ? e : simplifyError('unexpected_error', `${e}`)) });
        }
    })
);

export const databaseLiveRoutes = ({ projectName, accessKey }) => [
    _listenCollection,
    _listenDocument,
    _startDisconnectWriteTask
].map(route =>
    handleSocketPlug(route, async (socket, _response, roof) => {
        ...
        if (route === _listenDocument || route === _listenCollection) {
            ...
            try {
                ...
                socket.on('disconnect', async () => {
                    if (roof.writeTask) {
                        ...
                        DisconnectionWriteTaskListener[projectName].triggerListener({
                            status: 'completed',
                            committed,
                            task: roof.writeTask
                        });
            ...
            DisconnectionWriteTaskListener[projectName].triggerListener({ status: 'cancelled', task: roof.writeTask });
            roof.writeTask = undefined;
            socket.disconnect();
        }
    })

...
export const emitDatabase = (path, callback, options) => {
    const { includeBeforeData, includeAfterData, dbName, dbUrl, pipeline } = options || {};

    const col = getDB(this.projectName, dbName, dbUrl).collection(path),
...
export const databaseLiveRoutes = ({ projectName }) => [
    '_listenQuery',
    '_listenDocument'
].map(route =>
    handleSocketPlug(route, (socket, response) => {

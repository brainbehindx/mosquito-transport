...
export const databaseLiveRoutes = ({ projectName, accessKey, logger }) => [
    ...
    handleSocketPlug(route, async (socket, roof, acknowledged) => {
        ...
        const { mtoken, ugly, accessKey: ak, __: initBody } = socket.handshake.auth,
            [auth, nauth] = mtoken ? await Promise.all([
                niceTry(() => validateJWT(mtoken, projectName)),
                niceTry(() => verifyJWT(mtoken, projectName))
            ]) : [];
        ...
        if (decryptString(ak || '', accessKey, '_') === accessKey) {
            ...
            if (ugly) {
                if ((mtoken && nauth) || !mtoken) {
                    reqBody = decryptString(initBody, accessKey, mtoken ? nauth?.encryptionKey : accessKey);
                    if (reqBody) {
                        reqBody = JSON.parse(initBody);

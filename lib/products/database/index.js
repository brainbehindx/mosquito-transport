...
import { set as setLodash, get as getLodash, unset as unsetLodash } from 'lodash';
    ...
    tv.forEach((e, i) => {
        queryEntries(e).forEach(([key, value]) => {
            ...
            if (key.endsWith('$timestamp') && value === 'now') {
                setLodash(result[i], l.join('.'), Date.now());
            } else if (key.endsWith('$increment')) {
                setLodash(result[i], l.map((v, i, a) => i === a.length - 1 ? `$inc.${v}` : v).join('.'), value || 1);
            } else if (key.endsWith('$deletion')) {
                setLodash(unset[i], `${l.join('.')}`, value);
            } else setLodash(result[i], key, value);
        });

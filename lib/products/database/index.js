...
import { IS_WHOLE_NUMBER, decryptString, encryptString, niceTry, queryEntries, simplifyCaughtError, simplifyError } from "../../helpers/utils.js";
...
import { validateJWT, verifyJWT } from "../auth/tokenizer.js";
...
export const databaseRoutes = ({ projectName, logger, accessKey }) => [
    ...
    express.Router({ caseSensitive: true }).post(`/${mroute}`, async (req, res) => {
        ...
        try {
            ...
            const { commands, dbName, dbUrl } = reqBody,
                ...
                operation = route === '_readDocument' ? 'findOne' : route === '_queryCollection' ? 'findMany' : route === '_writeDocument' ? (scope || 'setOne') : 'unknown';
...
export const databaseLiveRoutes = ({ projectName, accessKey, logger }) => databaseLivePath.map(route =>
    handleSocketPlug(route, async (socket, roof, acknowledged) => {
        const hasLogger = logger.includes('all') || logger.includes('database'),
            now = Date.now();

        if (hasLogger) console.log(`started route: /${route}`);

        const { mtoken, ugly, accessKey: ak, __: initBody } = socket.handshake.auth,
            auth = mtoken ? await niceTry(() => validateJWT(mtoken, projectName)) : undefined;
        ...
        if (decryptString(ak || '', accessKey, '_') === accessKey) {
            ...
            if (ugly) {
                if ((mtoken && auth) || !mtoken) {
                    reqBody = decryptString(initBody, accessKey, mtoken ? auth?.encryptionKey : accessKey);
                    if (reqBody) {
                        reqBody = JSON.parse(reqBody);
            ...
            try {
                ...
                const callSnapshot = async () => {
                    ...
                    if (!isEqual(a, lastEmittedValue))
                        socket.emit('mSnapshot', [undefined, a]);
                    lastEmittedValue = a;
                }
                ...
                }, projectName, dbName, dbUrl, { pipeline: { ...(isDocumentWatch ? findOne : find) } })

...
export const databaseRoutes = ({ projectName, logger, accessKey, enforceE2E_Encryption, castBSON, ddosMap, internals }) => [
    ...
    express.Router({ caseSensitive: true }).post(`/${mroute}`, async (req, res) => {
        ...
        try {
            if (internals?.database === false) throw ERRORS.DISABLE_FEATURE;
            const ddosRouting = {
                [_readDocument]: 'read',
            ...
            switch (route) {
                case _readDocument:
                    ...
                    res.status(200).send(makeResult({ status: 'success', result: serializeToBase64({ _: result }) }));
                    break;
                case _queryCollection:
                    ...
                    res.status(200).send(makeResult({ status: 'success', result: serializeToBase64({ _: result1 }) }));
                    break;
                case _writeDocument:
                    const statusData = await writeDocument(commands, projectName, dbName, dbUrl);
                    res.status(200).send(makeResult({ status: 'success', statusData }));
                    break;
                case _writeMapDocument:
                    const committed1 = await writeMapDocument(commands, projectName, dbName, dbUrl);
                    res.status(200).send(makeResult({ status: 'success', statusData: committed1 }));
                    break;
                case _documentCount:
            ...
            try {
                ...
                const callSnapshot = async () => {
                    ...
                    if (newHash !== lastEmittedValue) {
                        const s = ugly ? serializeE2E(newHash, clientPublicKey, projectName) : newHash;

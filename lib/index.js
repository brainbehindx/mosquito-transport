    ...
    io.on('connection', async socket => {
        ...
        try {
                    ...
                    socket.on(route, function () {
                        ...
                        (async function () {
                            ...
                            callback?.(...reqBody, ...typeof emitable === 'function' ? [async function () {
                                ...
                                if (ugly) {
                                    res = await serializeE2E(stringify(args), clientPublicKey, projectName);
                                } else res = args;
                                emitable(res);
                            }] : []);
                        })();
            ...
            const emitEvent = async ({ emittion, timeout, promise }) => {
                ...
                const reqBuilder = ugly ? await serializeE2E(stringify(mit), clientPublicKey, projectName) : null;
                ...
                const result = await (timeout ? socket.timeout(timeout) : socket)[promise ? 'emitWithAck' : 'emit'](
                    ...
                    ...hasEmitable ? [function () {
                        ...
                        (async function () {
                            ...
                            if (ugly) {
                                res = parse((await deserializeE2E(args, projectName))[0]);
                            } else res = args;
                ...
                if (result && promise) return ugly ? parse((await deserializeE2E(result, projectName))[0])[0] : result[0];
            };
    ...
    listenHttpsRequest = (route = '', callback, options) => {
        ...
        Scoped.expressInstances[this.port].use(
            express.Router({ caseSensitive: true }).all(`/${normalizeRoute(route)}`, async (req, res) => {
                ...
                try {
                    useDDOS(this.config.ddosMap, route, req.ip, 'requests');
                } catch (_) {
                    res.setHeader(
                        'simple_error',

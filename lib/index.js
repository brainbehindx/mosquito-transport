...
import { IS_JSON_OBJECT, IS_RAW_OBJECT, IS_WHOLE_NUMBER, deserializeE2E, getStringExtension, niceTry, requestURL, serializeE2E, simplifyCaughtError, simplifyError } from "./helpers/utils.js";
...
import { ADMIN_DB_NAME, ADMIN_DB_URL, EngineRoutes, STORAGE_PATH, STORAGE_PREFIX_PATH, STORAGE_ROUTE, STORAGE_URL_TO_FILE, one_hour, one_mb, one_minute } from "./helpers/values.js";
...
const serveStorage = ({
    ...
    if (typeof route === 'string' && route.startsWith(`${STORAGE_ROUTE}/`)) {
        ...
        const routeTransformer = mediaRoute === '*' ||
            ...
            filePath = `${STORAGE_PATH(projectName)}/${linkRef.pathname}`,
            partern = {};
        ...
        [
            ...
            const v = paths.map(v => ext(linkRef.searchParams.get(v))).filter(v =>
                v !== undefined
            )[0];
            ...
            try {
                if (localBuffer) {
                    if (routeTransformer?.transform) {
                        ...
                        const { width, height, grayscale, blur, fit, top, left, flip, flop, format, quality, lossless, mute } = partern,
                            SharpLib = mediaType === 'image' ? require('sharp') : undefined;
                        ...
                        if (mediaType === 'image') {
                            ...
                            if (format) sharpInstance = sharpInstance.toFormat(format, { lossless, quality });

                            rib = await sharpInstance.toBuffer();
                        } else {
                            const com = [],
                                crf = quality ? ' -crf ' + (quality / 2) : '',
                                inputPath = STORAGE_URL_TO_FILE(linkRef.href, projectName),
                                outTipDir = `${STORAGE_PREFIX_PATH(projectName)}/.vid_freezer/${linkRef.pathname}`.split('/').filter((_, i, a) => i !== a.length - 1).join('/'),
                                outPath = `${outTipDir}/${encodeURIComponent(linkRef.href)}`;
                            ...
                            if (Scoped.cacheTranformVideoTimer[outPath]?.timer) {
                                ...
                                rib = await new Promise(async (resolve, reject) => {
                                    ...
                                    try {
                                        await mkdir(outTipDir);
                                    } catch (e) {
                                        reject(e);
                                        if (Scoped.cacheTranformVideoTimer[outPath]) {
                                            Scoped.cacheTranformVideoTimer[outPath].processList?.map?.(([_, deny]) => deny(e));
                                            delete Scoped.cacheTranformVideoTimer[outPath];
                                        }
                                        return;
                                    }

                                    exec(`ffmpeg -i ${inputPath}${mute ? ' -an' : ''} -vf "${com.join(', ')}"${mute ? '' : ' -c:a'} copy${crf} ${outPath}`, async (err) => {
                                        if (!Scoped.cacheTranformVideoTimer[outPath]) {
...
const useMosquitoDbServer = (app, config) => {
    const { projectName, port, accessKey, logger, staticContentCacheControl, staticContentMaxAge, staticContentProps, externalAddress, corsOrigin, maxRequestBufferSize, maxUploadBufferSize, onSocketSnapshot, enforceE2E_Encryption, transformMediaRoute } = config;
    ...
    io.on('connection', async socket => {
        ...
        try {
            ...
            ['on', 'once', 'prependOnceListener'].forEach(e => {
                listenersFuncObj[e] = (route, callback, onError) => {
                    ...
                    socket.on(route, async function () {
                        ...
                        callback?.(...reqBody, ...typeof restArgs === 'function' ? [function () {
            ...
            const emitEvent = ({ emittion, timeout, promise }) => {
                ...
                const lastEmit = restEmit.slice(-1)[0],
                    mit = typeof lastEmit === 'function' ? restEmit.slice(0, restEmit.length - 1) : restEmit;
                ...
                const p = await(isNaN(timeout) ? socket : socket.timeout(timeout))[promise ? 'emitWithAck' : 'emit'](
                    ...
                    ...typeof lastEmit === 'function' ? [function () {
                        ...
                        if (ugly) {
                            res = parse(deserializeE2E(args[0], projectName)[0]);
                        } else res = args;
                ...
                if (p) return ugly ? parse(deserializeE2E(p, projectName)[0])[0] : p[0];
            }
            ...
            const clonedSocket = {
                ...
                emitWithAck: function () {
                    emitEvent({ emittion: [...arguments], promise: true });
                },
                timeout: (timeout) => ({
                    emitWithAck: function () {
                        emitEvent({ emittion: [...arguments], timeout, promise: true });
                    }
                })
            };
...
export default class MosquitoTransportServer {
    constructor(configx) {
        ...
        Scoped.InstancesData[this.projectName] = {
            ...
            databaseRules: function () {
                const args = [...arguments];

                if (args[0]?.dbName === ADMIN_DB_NAME)
                    throw `Remote client are restricted from accessing dbName:${ADMIN_DB_NAME}`;

                return databaseRules?.(...args);
            },
        ...
        rmdir(`${STORAGE_PREFIX_PATH(projectName)}/.vid_freezer`, {
            recursive: true,
            force: true
        });
    }
                    ...
                    try {
                        const sendResult = (s, obj) => {
                            return s.send(uglified ? {
                                e2e: serializeE2E(IS_JSON_OBJECT(obj) ? JSON.stringify(obj) : obj, clientPublicKey, this.projectName)
                            } : obj);
                        }

                        res.send = sendResult.bind(res);
...
const validateServerConfig = (config, that) => {
    ...
    if (uidLength !== undefined && (!IS_WHOLE_NUMBER(uidLength) || uidLength < 10))
        throw `invalid value supplied to uidLength, expected a positive whole number greater than 9`;

    if (refreshTokenExpiry !== undefined && (
        !IS_WHOLE_NUMBER(refreshTokenExpiry) ||
        refreshTokenExpiry < one_hour ||
        (IS_WHOLE_NUMBER(accessTokenInterval) && accessTokenInterval >= refreshTokenExpiry)
    )) {
        if (IS_WHOLE_NUMBER(accessTokenInterval) && accessTokenInterval >= refreshTokenExpiry)
    ...
    if (accessTokenInterval !== undefined && (!IS_WHOLE_NUMBER(accessTokenInterval) || accessTokenInterval < one_minute * 10))
    ...
    if (dbUrl !== undefined && typeof dbUrl !== 'string') throw `dbUrl must be a string`;
    if (dbName !== undefined && typeof dbName !== 'string') throw `dbName must be a string`;
    ...
    if (sneakSignupAuth !== undefined && typeof sneakSignupAuth !== 'function') throw 'sneakSignupAuth must be a function';

...
export default class MosquitoTransportServer {
    ...
    extractBackup(config) {
        ...
        const { defaultName: adminName, instance: adminInstance } = mongoInstances.admin;
        const { defaultName, instance: defaultInstance } = mongoInstances.default;

        const adminUrl = `mongodb://${adminInstance.options.hosts}:${adminInstance.options.localPort}`,
            defaultUrl = `mongodb://${defaultInstance.options.hosts}:${defaultInstance.options.localPort}`;

        newConfig.database[adminUrl] = {
            [adminName]: '*'
        };
        if (!newConfig.database[defaultUrl])
            newConfig.database[defaultUrl] = {};
        if (!newConfig.database[defaultUrl][defaultName])
            newConfig.database[defaultUrl][defaultName] = '*';

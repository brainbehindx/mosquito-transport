                        ...
                        if (mediaType === 'image') {
                            ...
                            if (Scoped.cacheTranformVideoTimer[outPath]?.timer) {
                                ...
                                rib = await new Promise(async (resolve, reject) => {
                                    ...
                                    try {
                                        await mkdir(outTipDir, { recursive: true });
                                    } catch (e) {
                                        reject(e);
                                    ...
                                    // ultrafast
                                    const ffmpegCommad = `ffmpeg -i "${filePath}"${mute ? ' -an' : ''}${com.length ? ' -vf "' + com.join(', ') + '"' : ''}${mute ? '' : ' -c:a copy'}${crf} -preset superfast "${outPath}"`;
...
export default class MosquitoTransportServer {
    constructor(configx) {
        ...
        rmdir(`${STORAGE_PREFIX_PATH(this.projectName)}/.vid_freezer`, {
            recursive: true,
            force: true
        });
    }

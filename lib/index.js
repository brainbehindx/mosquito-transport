...
import { ADMIN_DB_NAME, ADMIN_DB_URL, EngineRoutes, STORAGE_PATH, STORAGE_ROUTE, STORAGE_URL_TO_FILE, one_mb, one_minute } from "./helpers/values.js";
...
const useMosquitoDbServer = (app, config) => {
    const { projectName, port, accessKey, logger, staticContentCacheControl, staticContentMaxAge, staticContentProps, externalAddress, corsOrigin, maxRequestBufferSize, maxUploadBufferSize, onSocketSnapshot } = config;
    ...
    io.on('connection', socket => {
        ...
        const { mtoken, ugly, accessKey: ak, isOutsider } = socket.handshake.auth;
        if (!isOutsider) return;

        const authRes = mtoken ? niceTry(() => validateJWT(mtoken, projectName)) : undefined,
            listenersFuncObj = {};
        ...
        ['on', 'once', 'prependOnceListener'].forEach(e => {
            listenersFuncObj[e] = (route, callback, onError) => {
                ...
                socket.on(route, async function () {
                    ...
                    let reqBody, residueError;

                    if (decryptString(ak || '', accessKey, '_') === accessKey) {

                        if (ugly) {
                            if ((mtoken && authRes) || !mtoken) {
                                reqBody = decryptString(emittion, accessKey, mtoken ? (await authRes)?.encryptionKey : accessKey);
                                if (reqBody) {
                                    reqBody = JSON.parse(reqBody);
                                } else residueError = simplifyError('decryption_failed', 'Decrypting request body failed');
                            } else residueError = simplifyError('invalid_auth_token', 'token provided is invalid');
                        } else reqBody = emittion;
                    } else residueError = simplifyError('incorrect_access_key', 'The accessKey been provided is not correct');

                    if (residueError) {
                        onError(residueError?.simpleError);
                    } else callback?.(...reqBody, ...restArgs);
                });
            }
        });
...
export default class MosquitoDbServer {
    constructor(configx) {
        ...
        Scoped.UidLength[this.projectName] = uidLength;
        Scoped.TokenExpiryInterval[this.projectName] = tokenRefreshInterval;
        this.config = config;
    }
...
const validateServerConfig = (config) => {
    ...
    if (tokenRefreshInterval !== undefined && (!IS_WHOLE_NUMBER(tokenRefreshInterval) || tokenRefreshInterval < one_minute))
        throw `invalid value supplied to tokenRefreshInterval, expected a positive whole number greater/equals than ${one_minute}(60 seconds)`;

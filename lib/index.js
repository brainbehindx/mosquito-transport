...
import { databaseLivePath, databaseLiveRoutes, databaseRoutes, dbRoute, emitDatabase, readDocument, writeDocument } from "./products/database/index.js";
...
import { cleanupPendingHashes, deleteDir, deleteSource, getSource, streamWritableSource, writeBuffer } from "./products/storage/store.js";
...
const serveStorage = ({
    ...
    if (typeof route === 'string' && route.startsWith(`${STORAGE_ROUTE}/`)) {
        ...
        try {
            useDDOS(ddosMap, 'get', req.ip, 'storage');
        } catch (error) {
            res.sendStatus(429);
            return;
        }
                        ...
                        if (mediaType === 'image' || routeTransformer?.transformAs === 'image') {
                            ...
                            if (Scoped.cacheTranformVideoTimer[outPath]?.timer) {
                                ...
                                rib = await new Promise(async (resolve, reject) => {
                                    ...
                                    const ffmpegCommad = `ffmpeg -i "${source}"${mute ? ' -an' : ''}${com.length ? ' -vf "' + com.join(', ') + '"' : ''}${mute ? '' : ' -c:a copy'}${crf}${vbr ? ' -b:v ' + vbr : ''}${abr ? ' -b:a' + abr : ''}${fps ? ' -r ' + fps : ''} -preset ${preset || 'medium'} "${await ensureDir(outPath)}"`;
        ...
        function sendFile(path) {
            res.sendFile(path, {
                ...
                if (err && (logger.includes('all') || logger.includes('error')))
...
const useMosquitoServer = (app, config) => {
    ...
    [
        ...
        (req, _, next) => {
            req.path = `/${normalizeRoute(req.path)}`;
            const url = new URL(req.url, 'http://_');
            url.pathname = req.path;
            req.url = url.href.substring(url.origin.length);
            next();
        },
        (req, _, next) => {
            if (!InternalRoutes.some(r => req.path.substring(1) === r)) {
        ...
        async (req, res, next) => {
            ...
            if (req.headers.uglified) {
                ...
                res.end = function (chunk) {
                    ...
                    originalWrite.call(res, typeof __sender === 'function' ? __sender(totalBuf) : totalBuf);
        ...
        try {
            ...
            if (ugly) {
                ...
                if (body.accessKey !== accessKey)
                    throw ERRORS.INCORRECT_ACCESS_KEY;
            ...
            ['on', 'once', 'prependOnceListener'].forEach(e => {
                listenersFuncObj[e] = (route, callback, onError) => {
                    ...
                    socket.on(route, async function () {
                        ...
                        callback?.(...reqBody, ...typeof emitable === 'function' ? [function () {
            ...
            const emitEvent = async ({ emittion, timeout, promise }) => {
                ...
                if (result && promise) return ugly ? parse(deserializeE2E(result, projectName)[0])[0] : result[0];
            };
...
export default class MosquitoTransportServer {
    ...
    listenHttpsRequest = (route = '', callback, options) => {
        Scoped.expressInstances[this.port].use(
            express.Router({ caseSensitive: true }).all(`/${normalizeRoute(route)}`, async (req, res) => {
                ...
                const { logger, accessKey } = this.config;
                ...
                try {
                    useDDOS(ddosMap, route, req.ip, 'requests');
                } catch (_) {
                    res.status(429).appendHeader(
                        'simple_error',
                        JSON.stringify(ERRORS.TOO_MANY_REQUEST.simpleError || {})
                    ).send({ status: 'error', ...ERRORS.TOO_MANY_REQUEST });
                    return;
                }
                ...
                try {
                    ...
                    res.status(statusErrorCode(e)).appendHeader(
                        'simple_error',
                ...
                try {
                    if (uglified) {
                        req.__sender = (buffer) => {
                            return serializeE2E(buffer.toString('base64'), clientPublicKey, this.projectName);
                        };
                    }
    ...
    streamBuffer = (destination, createHash, callback) => {
        validateStoragePath(destination);
        if (![undefined, true, false].includes(createHash))
            throw 'uploadBuffer() third argument must either be undefined or a boolean value';
    ...
    uploadBuffer = async (destination, buffer, createHash) => {
        validateStoragePath(destination);
    ...
    deleteFile = async (path = '') => {
        if (Validator.LINK(path)) {
            ...
            if (url.origin !== this.externalAddress)
                throw `Unknown origin, expected link with origin ${this.externalAddress} but got ${url.origin}`;
...
const validateServerConfig = (config, that) => {
    ...
    if (internals) {
        ...
        Object.entries(internals).forEach(([k, v]) => {
            ...
            if (typeof v !== 'boolean') throw `expected a boolean value at internals.${k} but got ${v}`;
        });
    }

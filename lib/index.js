...
const useMosquitoServer = (app, config) => {
    const { projectName, port, accessKey, logger, staticContentCacheControl, staticContentMaxAge, staticContentProps, externalAddress, corsOrigin, maxRequestBufferSize, maxUploadBufferSize, onSocketSnapshot, enforceE2E_Encryption, transformMediaRoute, preMiddlewares, transformMediaCleanupTimeout } = config;
...
export default class MosquitoTransportServer {
    ...
    updateUserMetadata = async (uid, metadata) => {
        if (!IS_RAW_OBJECT(metadata)) throw `metadata requires a raw object value`;
